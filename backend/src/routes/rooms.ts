...
const draftInput = z.object({
  ...
  effect: z.enum(["original", "echo", "reverb", "pitch"]).default("original"),
...
async function roomState(roomId: Types.ObjectId) {
  ...
  if (!room) return null;
  const members = await RoomMember.find({ roomId, isActive: true })
    .populate("userId", "name email avatar")
    .lean();
  const drafts = await Draft.find({ roomId }).lean();
  const activeSpin = await Spin.findOne({
    roomId,
    status: { $in: ["WAITING", "RUNNING"] },
  }).lean();
  const spinParticipants = activeSpin
    ? await SpinParticipant.find({ spinId: activeSpin._id })
...
export function createRoomRouter(io: Server) {
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (room.status === "COMPLETED") {
        res.status(409).json({ error: "Room is completed" });
        return;
      }
      const member = await RoomMember.findOneAndUpdate(
        { roomId: room._id, userId },
        {
          $set: { isActive: true, leftAt: undefined },
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const state = await roomState(
        new Types.ObjectId(String(req.params.roomId)),
      );
      if (!state) {
        res.status(404).json({ error: "Room not found" });
        return;
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!room || !member) {
        res.status(403).json({ error: "Active room membership required" });
        return;
      }
      ...
      io.to(room._id.toString()).emit("draft_shared", {
        draftId: draft._id,
        userId,
