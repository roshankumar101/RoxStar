...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const participations = await SpinParticipant.find({ userId })
      .sort({ joinedAt: -1 })
      .populate({ path: "spinId", populate: { path: "roomId" } })
      .lean();
    const createdRooms = await Room.find({ ownerId: userId })
      .sort({ createdAt: -1 })
      .lean();
    const wins = participations.filter((item) => item.status === "WINNER");
    const eliminations = participations.filter(
      (item) => item.status === "ELIMINATED",
    );
    res.json({
      statistics: {
        roomsJoined: memberships.length,
        spinsParticipated: participations.length,
        wins: wins.length,
